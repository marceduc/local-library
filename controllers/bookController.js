...
exports.book_create_get = function(req, res, next) {
  async.parallel({
    ...
    // Get all authors and genres which can be used for adding to our book.
  ...
  (req, res, next) => {
    ...
    if(!errors.isEmpty()){
      ...
      asynic.parallel({
        ...
        // Mark our select genre as checked.
        for (let i = 0; i < results.genres.length; i++){
          if (book.genre.indexOf(results.genres[i]._id > -1)){
            results.genres[i].checked='true';
          }
        }
        res.render('book_form', {title: 'Create Book', authors: results.authors, genres:result.genres, book: book, errors:errors.array()})
      });
      return;
    }
    else {
...
exports.book_delete_post = function(req, res, next) {
  ...
  async.parallel({
    ...
    if(results.book_instances.length > 0){
      // Book has bookinstances. Render in same way as GET route.
      res.render('book_delete', {title:'Delete Book', book:results.book, book_instances:results.book_instances} );
    }
    else{
      // Book has no instances. Delete object and redirect to lsit of books.
